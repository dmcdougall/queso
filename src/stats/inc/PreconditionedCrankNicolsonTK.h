...
/*!
 * \class PreconditionedCrankNicolsonTK
 *
 * \brief This class allows the representation of a transition kernel with a
 * scaled covariance matrix.
 */
template <class V = GslVector, class M = GslMatrix>
class PreconditionedCrankNicolsonTK : public BaseTKGroup<V, M> {
public:
  //! @name Constructor/Destructor methods
  //@{
  //! Default constructor.
  PreconditionedCrankNicolsonTK(const char * prefix,
                                const VectorSpace<V, M> & vectorSpace,
                                const std::vector<double> & scales,
                                const M & covMatrix);
  //! Destructor.
  ~PreconditionedCrankNicolsonTK();
  //@}
