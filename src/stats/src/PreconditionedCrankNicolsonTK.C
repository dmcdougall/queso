  ...
  m_originalCovMatrix(covMatrix)
{
  ...
  if ((m_env.subDisplayFile()) && (m_env.displayVerbosity() >= 5)) {
    *m_env.subDisplayFile() << "In PreconditionedCrankNicolsonTK<V, M>::constructor()"
  ...
  if ((m_env.subDisplayFile()) && (m_env.displayVerbosity() >= 5)) {
    *m_env.subDisplayFile() << "Leaving PreconditionedCrankNicolsonTK<V, M>::constructor()"
                           << std::endl;
  }
}
...
template <class V, class M>
bool
PreconditionedCrankNicolsonTK<V, M>::symmetric() const
{
  return true;
}
...
PreconditionedCrankNicolsonTK<V, M>::rv(unsigned int stageId) const
{
  ...
  if ((m_env.subDisplayFile()        ) &&
    ...
    *m_env.subDisplayFile() << "In PreconditionedCrankNicolsonTK<V, M>::rv1()"
                            << ", stageId = " << stageId
...
template <class V, class M>
const GaussianVectorRV<V, M> &
PreconditionedCrankNicolsonTK<V, M>::rv(const std::vector<unsigned int> & stageIds)
{
  ...
  if ((m_env.subDisplayFile()        ) &&
      (m_env.displayVerbosity() >= 10)) {
    *m_env.subDisplayFile() << "In PreconditionedCrankNicolsonTK<V, M>::rv2()"
...
template <class V, class M>
void
PreconditionedCrankNicolsonTK<V, M>::updateLawCovMatrix(const M & covMatrix)
{
  for (unsigned int i = 0; i < m_scales.size(); ++i) {
    double factor = 1./m_scales[i]/m_scales[i];
    if ((m_env.subDisplayFile()        ) &&
        (m_env.displayVerbosity() >= 10)) {
      *m_env.subDisplayFile() << "In PreconditionedCrankNicolsonTK<V, M>::updateLawCovMatrix()"
                              << ", m_scales.size() = " << m_scales.size()
                              << ", i = "               << i
...
template <class V, class M>
bool
PreconditionedCrankNicolsonTK<V, M>::setPreComputingPosition(const V & position, unsigned int stageId)
{
  if ((m_env.subDisplayFile()) && (m_env.displayVerbosity() >= 5)) {
    *m_env.subDisplayFile() << "Entering PreconditionedCrankNicolsonTK<V, M>::setPreComputingPosition()"
                           << ": position = " << position
                           << ", stageId = "  << stageId
                           << std::endl;
  }
  ...
  if ((m_env.subDisplayFile()) && (m_env.displayVerbosity() >= 5)) {
    *m_env.subDisplayFile() << "Leaving PreconditionedCrankNicolsonTK<V, M>::setPreComputingPosition()"
                           << ": position = " << position
                           << ", stageId = "  << stageId
                           << std::endl;
  }
...
template <class V, class M>
void
PreconditionedCrankNicolsonTK<V, M>::clearPreComputingPositions()
{
...
template <class V, class M>
void
PreconditionedCrankNicolsonTK<V, M>::setRVsWithZeroMean()
{
  queso_require_not_equal_to_msg(m_rvs.size(), 0, "m_rvs.size() = 0");
...
template <class V, class M>
void
PreconditionedCrankNicolsonTK<V, M>::print(std::ostream & os) const
{
  BaseTKGroup<V, M>::print(os);
}

template class PreconditionedCrankNicolsonTK<GslVector, GslMatrix>;
