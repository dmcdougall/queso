...
#ifndef LIBMESH_EIGEN_BLOCK_SPARSE_MATRIX_H
#define LIBMESH_EIGEN_BLOCK_SPARSE_MATRIX_H
...
/**
 * The EigenBlockSparseMatrix class wraps a sparse matrix object from the GSL
 * library.
 *
 ...
 */
template <typename T>
class EigenBlockSparseMatrix libmesh_final : public libMesh::SparseMatrix<T>
{
...
public:
  ...
  EigenBlockSparseMatrix(const std::vector<unsigned int> & blockSizes, const EigenSparseVector<T> & v, double diagValue);
  ...
  EigenSparseMatrix<T> & getBlock(unsigned int i) const;
   ...
   */
  virtual void close () const libmesh_override { const_cast<EigenBlockSparseMatrix<T> *>(this)->_closed = true; }
...
private:
  ...
  std::vector<QUESO::VectorSpace<EigenSparseVector<T>, EigenSparseMatrix<T> >* > m_vectorSpaces;
  std::vector<EigenSparseMatrix<T> *> m_blocks;
...
#endif // #ifdef LIBMESH_EIGEN_BLOCK_SPARSE_MATRIX_H
