...
[
  ...
  if (test x$enableeigen = xyes); then
    ...
    AC_ARG_WITH(eigen-include,
                AC_HELP_STRING([--with-eigen-include=PATH],[Specify the path for EIGEN header files (for example $EIGEN_PATH/include/eigen3)]),
    ...
    elif (test -f /usr/include/Eigen/Eigen); then
        EIGEN_INC="/usr/include"

    else
        AC_MSG_ERROR([
    
    EIGEN was requested but could not be found.  The path passed to
    --with-eigen-include should specify the location of EIGEN's headers.
    ...
    elif (test x$is_package_required = xyes); then
        AC_MSG_ERROR([

EIGEN is required but could not be found.  The path passed to
--with-eigen-include should specify the location of EIGEN's headers.
